  ...
  const renderLineWithEmailLinks = (line: string) => {
    ...
    return parts.map((part, index) => {
      if (emailRegex.test(part)) {
        return (
          <a
            onClick={(e) => { window.open(`mailto:${part}`, 'mail'); e.preventDefault() }}
            key={index}
            href={`mailto:${part}`}
            target="_blank"
            rel="noopener noreferrer"
    ...
    >
      <div className="bg-gray-800 px-2 py-1 text-xs text-gray-300 flex items-center absolute top-0 left-0 w-full select-none" style={{cursor: mouseOverClose ? 'pointer' : 'default'}}>
        <div className="flex space-x-1">
          <div ref={closeButtonRef} className="w-3 h-3 bg-red-500 rounded-full flex justify-center items-center"
            onMouseEnter={() => { setMouseOverClose(true); document.body.style.cursor = 'pointer' }}
            onMouseLeave={() => { setMouseOverClose(false); document.body.style.cursor = 'default' }}
          ...
          >{mouseOverClose &&
              <span className="text-center select-none text-black" style={{
                fontSize: '7pt',
                paddingRight: '1px',
                paddingBottom: '1px',
                fontWeight: '1000'
      ...
      <div className="p-2 h-full overflow-y-auto bg-black text-green-400 font-mono text-sm" ref={scrollContainerRef} onClick={handleClick}>
        ...
        {history.map((line, index) => (
          <div key={index} className="mb-1 cursor-text" onClick={(e) => e.stopPropagation()}>{renderLineWithLinks(line).map(line => {
