  ...
  return (
    <main className="flex min-h-screen flex-col sm:flex-row items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
      ...
      <div
        ...
        inline-flex justify-center items-center text-[rgba(204,102,255,0.5)]">
      ...
      <div className="w-2/3 sm:w-2/5 lg:w-1/4 flex flex-col items-center">
        {/* Error container */ error &&
          ...
          px-4 py-3 my-4 rounded relative" role="alert">
        ...
        {/* Message container */ message &&
          ...
          px-4 py-3 my-4 rounded relative" role="alert">
            ...
            <span className="block">{message.map(line => (<>{line} </>))}</span>
            <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
        ...
        {/* Link with {Provider} buttons */}
        {availableProviders.length ?
          <div className='flex flex-col'>
            {availableProviders.map(provider => (
              <div key={provider.id} className='flex-1'>
                <button className="rounded-xl w-full bg-white/20
                px-5 py-3 my-2 font-semibold text-white no-underline transition
              hover:bg-white/30"
                  onClick={() => void signIn(provider.id, {
                    callbackUrl: `/auth/linkaccount?message=${encodeURIComponent(`Your ${provider.name} account has been successfully linked.`)}`
                  })}>
                  <SocialIcon network={provider.id} className="mr-2"
        ...
        <Link href="/"><button className="rounded-full bg-white/10
          px-10 py-3 my-2 font-semibold text-white no-underline transition
        hover:bg-white/20">
          Go back</button></Link>
      </div>
    </main>
